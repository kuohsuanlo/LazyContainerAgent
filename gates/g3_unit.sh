...
[ "${nsucc:-0}" -ge 45 ] && ok "成功 ${nsucc} 個(≥45:確認測試真的有編進去)" || fail "只跑了 ${nsucc} 個測試,少於預期(≥45;是不是有測試沒編譯到?)"
for t in SummaryDifferentialTest EnsureRaceTest AttributionClassifyTest ComponentPartialSemanticsTest; do
