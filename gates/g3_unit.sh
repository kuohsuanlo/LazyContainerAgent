...
[ "${nsucc:-0}" -ge 55 ] && ok "成功 ${nsucc} 個(≥55:確認測試真的有編進去)" || fail "只跑了 ${nsucc} 個測試,少於預期(≥55;是不是有測試沒編譯到?)"
for t in SummaryDifferentialTest EnsureRaceTest AttributionClassifyTest ComponentPartialSemanticsTest RawSegmentsTest; do
