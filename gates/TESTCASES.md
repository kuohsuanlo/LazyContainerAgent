...
### 2026-09-09 keep raw 之後的實測(素材同上,26.2-7 定版)

| 回合 | 注入 | 磁碟歸零 | 警報 | 計數器 |
|---|---|---|---|---|
| `wipe`(物化過、沒人碰過) | 305 | **0** | `SILENT WIPE` 305 | `silentWipe=305/305`(偵測/自救)+ `SAFE MODE` |
| `wipeAccessed`(碰過之後,按 chunk 全清) | 1783(16 個 chunk) | 1783 | `MASS EMPTY` 15 個 chunk | `massEmpty=15/1782`;救援檔 15 個、裡面 1782 個容器 = 達門檻 15 個 chunk 的全部;第 16 個 chunk 只掉 1 個,門檻以下不落檔 |

`wipe` 那一行就是服主要的三件事同時成立:偵測到 → 用留著的原始資料重寫 → 大聲報錯,磁碟零流失。
`wipeAccessed` 那一行是刻意不自動寫回的:磁碟會掉,但救援檔一個都不少,還原是複製檔案。

### 2026-09-09 首跑實測(keep raw 之前;素材:s3 商場 `r.1.0`,15/15 綠)

| 回合 | 注入次數 | 磁碟歸零 | 磁碟減少 | 警報行 | 計數器 |
|---|---|---|---|---|---|
| `green` | 0 | 0 | 0 | 0 | `silentWipe=0/0` `badRaw=0` |
| `wipeKeepRaw` | 302 | **0** | 0 | 0 | 原樣寫回全接住,不需要警報 |
| `wipe` | 302 | 302 | 0 | 200(印行上限) | `silentWipe=302/0`、`SAFE MODE` |
| `wipe-noguard` | 302 | 302 | 0 | **0** | 關掉守門就完全沒有訊號 |
| `corruptRaw` | 302 | 95 | 25 | 60(印行上限) | `badRaw=96`、`SAFE MODE` |

兩個必須記住的界線:

1. **(keep raw 之前的界線,現已不成立)** 當時守門只在 raw 已經被物化吃掉之後才有機會出手,
   而那時候它救不回來(`silentWipe=302/**0**`)。26.2-7 定版起 raw 物化後留著,同一個故障變成
   `silentWipe=305/305`、磁碟零流失。留下這段是為了記得「煙霧偵測器」和「滅火器」的差別是怎麼被逼出來的。
2. **側車被吞的指紋是「缺 Items 鍵」,不是「空清單」。** `dropSideCar` 回合吞掉 565 個側車,
   磁碟上缺鍵數 118 → 683(剛好 +565),而「空清單 + 有東西」剛好少 565:565 個全部變成缺鍵,
   一個都沒變成空清單。這條把 2026-09-08 s3 事故的判讀從「讀原始碼推出來的」變成「做實驗量出來的」——
   那次是 152 個箱子寫成 `Items: []`、新增缺鍵 0 個,所以不是直寫造成的。

3. **位元翻轉如果翻成「仍然合法的 NBT」,結構檢查抓不到。** 302 次注入裡有 96 次被 `BAD RAW`
   當場抓住並落檔,但磁碟上共有 120 個容器內容改變。要擋住剩下那部分需要對 raw 做校驗和
   (載入時算、寫出前驗),那是 O(bytes) 的每次存檔成本,目前沒有做。
