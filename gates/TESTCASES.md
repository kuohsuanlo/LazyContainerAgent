...
### 2026-09-09 首跑實測(素材:s3 商場 `r.1.0`,26.2-7,15/15 綠)
...
1. **救資料的是原樣寫回,不是守門。** `wipeKeepRaw` 那一輪 302 個容器的記憶體清單被整個清空,
   磁碟零流失,因為沒被碰過的容器本來就是把載入時的 bytes 原樣寫回去。守門只在 raw 已經被
   物化吃掉之後才有機會出手,而那時候它救不回來(`silentWipe=302/**0**`),只能偵測 + 降級。
