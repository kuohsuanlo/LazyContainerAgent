...
for r in $ROUNDS; do
  [ -f "$E/$r.log" ] || continue
  printf "     %-13s FAULT=%-4s SILENT_WIPE=%-4s BAD_RAW=%-4s SAFE_MODE=%-3s silentWipe=%-6s badRaw=%-4s 歸零=%s\n" \
    "$r" "$(n "$r" 'FAULT ')" "$(n "$r" 'SILENT WIPE')" "$(n "$r" 'BAD RAW')" "$(n "$r" 'SAFE MODE')" \
    "$(ctrv "$r" silentWipe)" "$(ctrv "$r" badRaw)" "$(emptied "$r")"
done
...
case " $ROUNDS " in *" wipe "*)
  ...
  [ "$(ctrv wipe silentWipe)" -gt 0 ] && ok "[wipe] 計數器 silentWipe=$(ctrv wipe silentWipe)" || fail "[wipe] 計數器沒動"
