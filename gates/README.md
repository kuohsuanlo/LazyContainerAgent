...
> ⚠ **2026-09-12 revert:runtime 已退回 26.2-2(commit 8f6b40b),#261 直寫與其後的 keep raw / 寫入保真 / 守門 / SAFE MODE / 故障注入全部拔除。**
> 原因:26.2-5/7/8 三個版本上線都發生無帳本痕跡的大面積容器清空(09-08 s3;09-12 s3+s37),26.2-2 無事。根因未破,服主裁示不要這個功能。
> 現況:G0–G7 對 26.2-2 有效;`LC_HAS_PASSTHROUGH=0` 時 G4 只跑 V/A、直寫斷言跳過;**RED 不適用**(無故障注入旗標)。線上 9 台 = 26.2-2 原版 jar `64f1686b`。
> **命名:2026-09-12 起這份 runtime 叫 `26.2-9`(jar `5d942601`)= 26.2-2 的 class 逐檔相同(只差 MANIFEST/pom 版本號),差別只在這套 gate 系統已開發完成。線上 109 台跑的仍是 26.2-2 原檔 `64f1686b`,兩者可互換。26.2-3 ~ 26.2-8 一律不准鋪。**
...
| **寫入保真檢查 `verifyWrite`** | 26.2-7 | 服主指正的關鍵:**真相來源是當下記憶體裡那份清單**,不是載入時那份。玩家拿光,記憶體就是空的,那空的也是正確答案。所以不必分「碰過/沒碰過」,也沒有把玩家拿走的東西變回來的問題 —— 唯一要問的是「寫出去的東西跟記憶體現在說的一不一樣」。少了就是寫壞(寫空、寫少、寫到別的地方去),當場拆掉側車、用記憶體那份重寫一次 | `badWriteIsRepairedFromMemory`(寫壞要補回五格)、`emptyMemoryWritesEmptyWithoutAlarm`(記憶體空的不得報警)、`pendingContainerVerifiedAgainstRaw`(還沒物化的拿 raw 當真相)+ RED 的 `badWrite` 回合 | 記憶體筆數 = 還沒物化 ? `rawListSize(raw)` : 清單非空格數;寫出筆數 = 側車讀 ListTag 表頭 / Items 樹讀 size / 都沒有算 0(三種都不解析)。成本一趟 ≤27 格掃描 + 一個 size 讀取 | G3、RED |
| **存檔守門(靜默清空)+ keep raw 寫回 + 自動降級** | 26.2-7 | 2026-09-08 s3 商場單一 chunk 內 154 個容器同時歸零(面板 #121)。當時 agent 對「內容在沒人碰的情況下消失」零偵測能力。守門把它變成**存檔當下就叫的警報**;而且容器物化後 raw 不再立刻作廢,留到第一次存檔/卸載/10 分鐘,攔到就**用留著的 raw 寫回去**(服主要的:沒報錯就釋放、報錯就重新走原本的寫入),同時就地關掉直寫。「有人碰過之後才變空」分不出玩家拿光還是程式清空,**不寫回**(寫回 = 複製),改成整個 chunk 大面積歸零時把所有原始 bytes 落成可讀 NBT(`lc-massempty-*.nbt`)+ `MASS EMPTY` + 降級。⚠**預設關閉**(`-Dlazycontainer.massEmpty=true` 才開)——商店服(假人商店)的容器本來就會整片被買空,那是正常販售不是事故,這一層分不出來;真正可靠的是「寫壞」(寫入保真,恆對商店安靜)。開了之後門檻看佔比不看絕對數:一個 chunk 裡原本有貨的容器,超過一半(`massEmpty.fraction`,預設 0.5)同時被清空、且 ≥ `massEmpty.min`(40)才算——s3 事故 77%,正常玩家/漏斗搬幾箱是個位數 %(2026-09-10 s3/s100 誤報實測 2~7%)| `SilentWipeGuardTest` 13 例:物化後沒人碰→寫回並釋放、存檔正常→釋放、到期→釋放、碰過後變空→絕不寫回、≥8 個碰過後歸零→落檔+報警、<8→安靜、玩家取光/載入即空/setItems/還有東西/物化重入→全部安靜 | 熱路徑:guard 多一個 volatile 讀(標記 accessed);`silentWipe=偵測/自救`、`massEmpty=chunk數/容器數`、`keptRaw=目前留著的容器數`;RED 的 `wipe` 回合要磁碟零流失、`wipeAccessed` 回合要救援檔一個不少 | G3、G4、RED |
...
| `-Dlazycontainer.fault=corruptRaw\|wipe\|wipeKeepRaw\|wipeAccessed\|badWrite\|dropSideCar` | **故障注入,只給紅綠驗證台**。啟用時開機印一整段大字,G4 會 grep 那段字判紅 | RED |
